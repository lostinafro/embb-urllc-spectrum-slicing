...
class NOMAExhaustive(NOMAFeasible):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(NOMAExhaustive, self).__init__(seed=seed,
    ...
    def get_feasible_initial_point(self):
        ...
        self.embb_power_allocation()
        # 2. URLLC power allocation for SIC and IL purpose
        self.urllc_sic_power_allocation()
        self.urllc_il_power_allocation()
    ...
    def exhaustive_search(self,
        ...
        # Direction matrix
        order = np.argsort(self.embb_power[0])[::-1]
        urllc_freqs = np.flatnonzero(self.urllc_allocation)
        for o in order:
            if o not in urllc_freqs:
                order = order[order != o]
...
if __name__ == "__main__":
    ...
    # Output file
    if render:
        output_csv = output_csv_path('noma_exhaustive_metrics', 'optimization', Fu, Mu, suffix)
    ...
    for snr in mean_snr_list:
        ...
        for s in standard_bar(iterations):
            ...
            env = NOMAExhaustive(s, re, ru, outage_target, snr, Fu, Mu, outage_df, verbose=verbose,
