...
import common.method as common
from scenario.cells import Cell
...
#   - implement the configuration parameter for shadowing
#   - create a wrapper that bang the creation of the simulation environment in the right
#             order (the most efficient one)
#   - insert function to do the placing from here
...
class CellCluster:
    ...
    def __init__(self, rb: Resources,
        ...
        :param rb:  Resources, instance of class resources giving frequency and time resources
                    available in the whole cluster.
        :param rng: numpy.random.RandomState, seed of the pseudorandom number generator for the simulation.
        :param frau: float, upper bound of the distance considered for the Fraunhofer region
        """
        # Parameters
        self.RB = rb            # store the resources available in the cluster
        self.FR = sum([x.N_sc for x in self.RB])
        self.cell = []          # store the cells considered in the cluster
        self.chan_gain = None   # store the channel gain tensor
        self.frau = frau        # store the distance assumed for the Fraunhofer region
    ...
    def place_cell(self, n: int,
        ...
        if not isinstance(sh_std, (tuple, list)):
            sh_std = [sh_std] * n
        for i in range(n):
            self.cell.append(Cell(r_outer[i], r_inner[i], coord[i], pl_exp[i], sh_std[i], n_old + i, rng=self.rng))
            if n_old + i - 1 >= 0:
    ...
    @property
    def nodes(self):
        return np.array([n for c in self.cell for n in c.node])

    @property
    def noise_power(self):
        return np.array([n.noise.linear for n in self.nodes])
    ...
    def get_user(self, partition: str = None) -> []:
        ...
        if partition in dic.traffic_types:
            return [n for n in self.nodes
                    if (n.type == 'UE' or (n.type == 'D2D' and n.dir == 'UL')) and n.traffic == partition]
        elif partition in dic.user_types:
            return [n for n in self.nodes if n.type == partition]
        elif partition in dic.user_dir:
            return [n for n in self.nodes if n.type in dic.user_types and n.dir == partition]
        else:
            return [n for n in self.nodes if n.type == 'UE' or (n.type == 'D2D' and n.dir == 'UL')]

    def get_bs(self, partition: str = None) -> []:
        """The function output a list of bs in the cluster."""
        if partition in dic.bs_types:
            return [n for n in self.nodes if n.type == partition]
        elif partition in dic.bs_dir:
            return [n for n in self.nodes if n.type in dic.bs_types and n.dir == partition]
        else:
            return [n for n in self.nodes if n.type in ('mBS', 'fBS')]

    def get_user_id(self, partition: str = None) -> np.ndarray:
        """The function output a list of users id in the cluster.
        TODO: not working for multi cell environment
        TODO: export a boolean vector
        """
        if partition in dic.traffic_types:
            return np.array([sum(n.id) for n in self.nodes
                            if (n.type == 'UE' or (n.type == 'D2D' and n.dir == 'UL')) and n.traffic == partition])
        elif partition in dic.user_types:
            return np.array([sum(n.id) for n in self.nodes if n.type == partition])
        elif partition in dic.user_dir:
            return np.array([sum(n.id) for n in self.nodes if n.type in dic.user_types and n.dir == partition])
        else:
            return np.array([sum(n.id) for n in self.nodes if n.type == 'UE' or (n.type == 'D2D' and n.dir == 'UL')])
            ...
            for i, n_i in enumerate(nodes):  # i is the receiver
                ...
                for j, n_j in enumerate(nodes[i:], i):  # j is the transmitter
                    ...
                    if j != i:  # channel is modeled as a Rayleigh fading
                        ...
                        # Let the pieces fit
                        data[f, j, i][np.ix_(range(n_i.ant), range(n_j.ant))] = fad * np.sqrt(10 ** (-(pl + sh) / 10))
                    elif n_j.dir == 'FD':  # j == i
                        # Full Duplex fading is Rice distributed
                        fad = common.fading("Rice", dim=(n_i.ant, n_j.ant), shape=n_j.r_shape, seed=s)
                        data[f, j, i][np.ix_(range(n_i.ant), range(n_j.ant))] = fad * np.sqrt(10 ** (n_i.si_coef / 10))
                    else:  # j == 1 dir != 'FD'
    ...
    def plot_scenario(self):
        ...
        for c in self.cell:
            ...
            # User positioning
            delta = c.r_outer / 100
            plt.text(c.coord[0] + c.r_outer / 2, c.coord[1] + c.r_outer / 2, s=f'cell {c.id}', fontsize=11, c='#D7D7D7')
            for n in c.node:
