...
# TODO:
#  - Implement the possibility of choosing noise
#  - Change the QoS in a **kwargs statement


class Node:
    """Node definition"""

    def __init__(self, typ: str, dire: str, coord: np.ndarray,
                 ant: int, gain: float, max_pow: float, noise_power: float):
        ...
        self.ord = None
        # Noise definition
        self.noise = RxNoise(dBm=noise_power) if self.dir != 'UL' else None
...
class Base(Node):
    ...
    def __init__(self, typ: str, dire: str, coord: np.ndarray,
        ...
        if noise_power is None:
            noise_power = -92.5  # [dBm]
        # Init parent class
        super(Base, self).__init__(typ, dire, coord, ant, gain, max_pow, noise_power)
        # Set attributes
...
class User(Node):
    ...
    #  and Pj = Tr{BjBj^H}
    def __init__(self, typ: str, dire: str, coord: np.ndarray,
                 ant: int = None, gain: float = None, max_pow: float = None, noise_power: float = None,
                 weight: float = None, traffic: str = None, qos: dict = None):
        ...
        qos : dict,
            ...
            if traffic is eMBB, QoS is (min_throughput, weight);
        ...
        try:  # Try to insert the standard qos keys values into qos
            for sk in qos_standard.keys():
                if sk not in qos.keys():
                    qos[sk] = qos_standard[sk]
        except AttributeError:
            qos = qos_standard
        # Init parent class
        super(User, self).__init__(typ, dire, coord, ant, gain, max_pow, noise_power)
        # User parameters
    ...
    #     user = fun.get_users()
    #     sub = chan.shape[0]
    #     # All the transmitters which interfere with snr(self) and their antennas
    ...
    #     snr = [self.useful.ord if self.dir == 'UL' else self.ord]
    #     ar = self.useful.ant if self.dir == 'UL' else self.ant
    #     # Noise seen
    #     noise = (self.useful.noise.linear if self.dir == 'UL'
    #              else self.noise.linear)
    #     # Channel gains between t_int to snr
    #     h = np.squeeze(chan[np.ix_(range(sub), t_int, snr)], axis=2)
