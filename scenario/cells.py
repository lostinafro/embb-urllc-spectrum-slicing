...
from scenario.nodes import User, Base


# TODO:
#   - Implement the positioning based on given path loss
#   - THIS SHOULD BE ALL A SUBCLASS OF PANDAS
...
class Cell:
    ...
    def place_bs(self, n: int, typ: str,
                 ...
                 si_coef=None, r_shape=None, noise_power=None):
        ...
        d : str in bs_directions and represents the duplex technology to be considered.
        ...
        max_pow : list of int or float, representing the maximum power
                  ...
                  same max_pow.
        si_coef : list of float or float, representing the self-interference
                  cancellation factor (FD mode); if a single number, each bs
                  will have same si_coef.
        r_shape : list of float or float, representing the ratio of the power
                  contributions by LOS path to the remaining multi-paths for SI
                  (FD node only); if a single number, each bs will have same
                  r_shape.
        noise_power : list of float or float, representing the noise power in dBm of the RF chain
        """
        # Control on INPUT
        ...
        for i in range(n):
            self.node.append(Base(typ=typ, dire=dire[i],
                                  ...
                                  gain=gain[i], max_pow=max_pow[i], noise_power=noise_power[i],
    ...
    def place_user(self, n: int, typ: str,
                   d=None, coord=None, ant=None, gain=None, max_pow=None, noise_power=None,
        ...
        # In case of D2D couple
            ...
            if coord is None:
                ...
                try:
                    ...
                    coord[rho > self.r_outer, :] = self.r_outer * np.hstack((np.cos(phi[rho > self.r_outer]),
